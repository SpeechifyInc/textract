...
const TEST_CASES = [
  ...
  ['text/*', 'txt.txt', 'This is a plain old text file.'],
  ...
  ['odt', 'odt.odt', 'This is an ODT THIS IS A HEADING More ODT'],
...
describe('fromUrl tests', { timeout: 3000 }, () => {
  ...
  it.each(TEST_CASES)('will %s files', (_ext, name, text) => {
    const url = `https://cdn.rawgit.com/dbashford/textract/master/test/files/${
      name
    }?raw=true`;
    fromUrl(url, (error, _text) => {
      expect(error).toBeNull();
      expect(text).toBeInstanceOf(String);
